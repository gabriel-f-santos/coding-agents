...
**Handoff → product-discovery.** Brainstorming is the divergent front-end; it does not validate.
When the PM converges on a direction, close by stating the **chosen hypothesis** ("Acredito que
[público] tem dificuldade em [dor]") plus the **riskiest assumptions to test**. That pair is the
input to `product-discovery`, which validates it with real evidence (Mom Test interviews, market
research) and writes the discovery brief. Don't carry an idea straight to a PRD — validate first.
